...
%     end
    ...
    methods
        function [power_out,map_out, evaluationGrid_x, evaluationGrid_y, source_loc,estimatedPilotSignals] = generateMap(obj)
            ...
            for ind_xr = 1:obj.n_gridpoints_x
                for ind_yr = 1:obj.n_gridpoints_y
                    [distance6ray,  powerDelayProfile,totalReceivedPowerAllSources, totalReceivedPower, H_D] = ...
                    ...
                    %                     estimatedDistances(ind_xr, ind_yr,:) = estimatedToaRange;
                    estimatedPilotSignals(:,:,ind_xr, ind_yr) =H_D; % allCenterOfMass;
                    
                    [ totalChannelAllSources,totalRxPowerlAllSources_2, totalChannel,totalRxPower_2] =...
                        obj.calculateCGGivenReceiverLocation(evaluationGrid_x(ind_xr, ind_yr), evaluationGrid_y(ind_xr,ind_yr));
                    channel_out_allS(:,ind_xr, ind_yr)=totalChannelAllSources;
                    channel_out(ind_xr, ind_yr) = totalChannel;
                    power_out_2(:,ind_xr, ind_yr)=totalRxPowerlAllSources_2;
                    map_out_2(ind_xr, ind_yr) = totalRxPower_2;
        ...
        function [h_D,samplingTime] = digitalImpulseResponse(obj, alpha, delays)
            ...
            maximumSamplingTime=ceil(max(delays)/obj.sampling_period); % 10 added is for ensuring some margin, so that all rays are considered
