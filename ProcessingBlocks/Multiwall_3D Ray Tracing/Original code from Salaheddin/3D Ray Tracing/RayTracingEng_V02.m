% 3D Ray Tracing Engin Only (All parameters are passed through workspace)
...
if reflectionFlag == 1
    ...
    if plotMode == 1
        ...
        %     [min(abs(Rx.TotalRSSILayer(:,i))),max(abs(Rx.TotalRSSILayer(:,i)))]
            colorbarLabels = min((Rx.TotalRSSILayer(:,i))) + (0:5) .* ((max(Rx.TotalRSSILayer(:,i))-min(Rx.TotalRSSILayer(:,i)))./5);
            colorbar('YTickLabel',num2str((colorbarLabels'),'%10.1f'));
            title(['First Reflections Only@ Z-Plane height of ',num2str(zplaneHeight(i),'%10.2f'),'; Tx Power = ',num2str(Tx.power')]);
...
%             imageRSSIScaledOverlayed = imoverlay(imageRSSIScaledOverlayed,blackageMaskScaled,[0,0,0])
        ...
        %     [min(abs(Rx.TotalRSSILayer(:,i))),max(abs(Rx.TotalRSSILayer(:,i)))]
            ...
            colorbar('YTickLabel',num2str((colorbarLabels'),'%10.1f'));
            title(['Second Reflections Only@ Z-Plane height of ',num2str(zplaneHeight(i),'%10.2f'),'; Tx Power = ',num2str(Tx.power')]);
...
if plotMode == 1
    ...
    %     [min(abs(Rx.TotalRSSILayer(:,i))),max(abs(Rx.TotalRSSILayer(:,i)))]
        colorbarLabels = min((Rx.TotalRSSILayer(:,i))) + (0:5) .* ((max(Rx.TotalRSSILayer(:,i))-min(Rx.TotalRSSILayer(:,i)))./5);
        colorbar('YTickLabel',num2str((colorbarLabels'),'%10.1f'));
        title(['Z-Plane height of ',num2str(zplaneHeight(i),'%10.2f'),'; LOS = ',num2str(losFlag),'; Reflec = ',num2str(reflectionFlag),'; Tx Power = ',num2str(Tx.power')]);
